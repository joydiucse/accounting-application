<x-app-layout>
    <x-slot name="header">
        <div class="flex justify-between items-center">
            <h2 class="font-semibold text-xl text-gray-800 leading-tight">
                {{ __('Dollar Expense Management') }}
            </h2>
            @if(auth()->user()->canManage())
                <a href="{{ route('dollar-expenses.create') }}" class="inline-flex items-center px-4 py-2 bg-red-600 border border-transparent rounded-md font-semibold text-xs text-white uppercase tracking-widest hover:bg-red-700 focus:bg-red-700 active:bg-red-900 focus:outline-none focus:ring-2 focus:ring-red-500 focus:ring-offset-2 transition ease-in-out duration-150">
                    Add New Dollar Expense
                </a>
            @endif
        </div>
    </x-slot>
    ...
    <div class="space-y-6">
        ...
        <div class="bg-white overflow-hidden shadow-sm rounded-lg">
            <div class="p-6">
                <form method="GET" action="{{ route('dollar-expenses.index') }}" class="grid grid-cols-1 md:grid-cols-4 gap-4">
                    <div>
        ...
        <div class="bg-white overflow-hidden shadow-sm rounded-lg">
            <div class="p-6">
                    ...
                    <table class="min-w-full divide-y divide-gray-200">
                        <thead class="bg-gray-50">
                            <tr>
                                ...
                                <th class="px-6 py-3 text-left text-xs font-medium text-gray-500 uppercase tracking-wider">BDT Amount</th>
                        ...
                        <tbody class="bg-white divide-y divide-gray-200">
                            @forelse($dollarExpenses as $expense)
                                <tr>
                                    ...
                                    @if(auth()->user()->canManage())
                                        <td class="px-6 py-4 whitespace-nowrap text-sm font-medium">
                                            <div class="flex space-x-2">
                                                <a href="{{ route('dollar-expenses.show', $expense) }}" class="text-blue-600 hover:text-blue-900">View</a>
                                                <a href="{{ route('dollar-expenses.edit', $expense) }}" class="text-indigo-600 hover:text-indigo-900">Edit</a>
                                                <form action="{{ route('dollar-expenses.destroy', $expense) }}" method="POST" class="inline" onsubmit="return confirm('Are you sure you want to delete this dollar expense record?')">
                                                    @csrf
                                                    @method('DELETE')
                            ...
                            @empty
                                <tr>
                                    <td colspan="{{ auth()->user()->canManage() ? '7' : '6' }}" class="px-6 py-4 text-center text-sm text-gray-500">
                                        No dollar expense records found.
                                    </td>
                                </tr>
                            @endforelse
                        </tbody>
                ...
                <!-- Pagination -->
                <div class="mt-4">
                    {{ $dollarExpenses->appends(request()->query())->links() }}
                </div>
                ...
                <div class="mt-4 bg-gray-50 p-4 rounded-lg">
                    <div class="grid grid-cols-1 md:grid-cols-2 gap-4 text-sm text-gray-600">
                        <div>
                            Total USD Expenses: <span class="font-semibold text-red-600">${{ number_format($totalUsdExpenses, 2) }}</span>
                        </div>
                        <div>
                            Total BDT Expenses: <span class="font-semibold text-blue-600">৳{{ number_format($totalBdtExpenses, 2) }}</span>
                        </div>
                        <div class="md:col-span-2">
                            ({{ $dollarExpenses->total() }} records)
                        </div>
                    </div>
                </div>
            </div>

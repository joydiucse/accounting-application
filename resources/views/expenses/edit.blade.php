...
function checkDollarBalance(requestedAmount) {
    ...
    fetch('/api/dollar-balance')
        ...
        .then(data => {
            ...
            // For edit form, we need to account for the current expense amount
            const currentExpenseAmount = {{ $expense->from_dollar && $expense->usd_amount ? $expense->usd_amount : 0 }};
            const availableBalance = data.balance + currentExpenseAmount;
            
            if (requestedAmount > availableBalance) {
                balanceElement.innerHTML = `<span class="text-red-600 font-medium">⚠️ Insufficient balance! Available: $${(availableBalance).toFixed(2)}</span>`;
                balanceElement.className = 'mt-2 text-sm text-red-600';
                submitButton.disabled = true;
                submitButton.classList.add('opacity-50', 'cursor-not-allowed');
            } else {
                balanceElement.innerHTML = `<span class="text-green-600">✓ Available balance: $${(availableBalance).toFixed(2)}</span>`;
